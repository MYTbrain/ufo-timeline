...
def write_witness_count_manifest_stub(root: Path) -> None:
    manifest = {
        "releaseId": "analysis-witness-count-test-v1",
        "assetBaseUrl": "https://assets.example.org/releases/analysis-witness-count-test-v1",
        "delivery": {
            "pagesFiles": ["manifest.json"],
    ...
    path = root / "data" / "analysis_witness_count_v1" / "manifest.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(manifest), encoding="utf-8")
...
def write_analysis_v2_manifest_stub(root: Path) -> None:
    manifest = {
        "releaseId": "analysis-v2-test-v1",
        "assetBaseUrl": "https://assets.example.org/releases/analysis-v2-test-v1",
        "delivery": {
            "pagesFiles": ["manifest.json"],
    ...
    path = root / "data" / "analysis_v2" / "manifest.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(manifest), encoding="utf-8")
...
def test_cloudflare_bundle_release_mode_enforces_exact_inventory(tmp_path, monkeypatch):
    ...
    write_analysis_v2_manifest_stub(bundle_root)
    (bundle_root / "cloudflare_bundle_manifest.json").write_text(
        json.dumps({"pages_safe": True}), encoding="utf-8"
    )
    (bundle_root / "r2_upload_manifest.json").write_text(
