...
from scripts.promote_remaining_lower_time_format_review_to_decision_candidates import (
    PROMOTION_POLICY,
    build_remaining_lower_time_format_decision_candidates,
)
...
def _review_item(review_item_id="er_cluster_a", *, recommendation="source_review_same_event_candidate", failed=None):
    return {
        ...
        "time_tokens": ["1900", "1910", "Even"],
        "parsed_minutes": [1140, 1150],
        "minute_span": 10,
        "fuzzy_labels": ["evening"],
        "active_conflicts": ["time"],
        "review_reason_codes": ["remaining_lower_time_format_consistent_source_evidence"],
        "source_names": ["ufocat"],
        "source_native_ids": ["native_1"],
        "dates": ["1957-11-10"],
        "locations": ["HAMMOND, Lake, IN, US"],
        "coordinate_values": ["41.61,87.51"],
        "failed_conditions": failed or [],
        "merge_canonical_event_ids": ["evt_a", "evt_b", "evt_c"],
        "candidate_canonical_input_ids": ["cin_a", "cin_b", "cin_c"],
        "notes": "reviewed",
    }
...
def test_promote_remaining_lower_time_format_review_writes_same_event_candidates():
    candidates, report = build_remaining_lower_time_format_decision_candidates(
        _report(
            _review_item("safe"),
    ...
    assert candidate["review_band"] == "remaining_lower_time_format_source_review"
    assert candidate["requires_explicit_apply_step"] is True
    assert candidate["evidence"]["parsed_minutes"] == [1140, 1150]


def test_promote_remaining_lower_time_format_review_skips_failed_same_event_item():
    candidates, report = build_remaining_lower_time_format_decision_candidates(
...
def test_promote_remaining_lower_time_format_review_rejects_unsafe_report():
    report = _report(_review_item())
    report["auto_merge_performed"] = True

    with pytest.raises(ValueError, match="auto_merge_performed"):
        build_remaining_lower_time_format_decision_candidates(report)
