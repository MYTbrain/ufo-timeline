"""Review medium body-text-mixed manual-review replacement rows.

Targets rows with description/summary conflicts mixed with time, location, or
classification conflicts, excluding identity and coordinate lanes already
packetized. Output is review-only; no decisions, apply, or canonical mutation.
"""
...
DEFAULT_JSON_OUTPUT = Path("data/reports/manual_review_ai_after_time_norm_medium_body_text_mixed_review.json")
DEFAULT_CSV_OUTPUT = Path("data/reports/manual_review_ai_after_time_norm_medium_body_text_mixed_review.csv")
DEFAULT_MARKDOWN_OUTPUT = Path("data/reports/manual_review_ai_after_time_norm_medium_body_text_mixed_review.md")

REVIEW_POLICY = "manual_review_medium_body_text_mixed_review_v1"
NEEDS_DEEPER_BODY_MIXED_REVIEW = "needs_deeper_body_mixed_review"

CSV_FIELDS = (
    "review_rank",
    "replacement_event_id",
    "review_recommendation",
    "body_mixed_subcategory",
    "confidence",
...
def build_medium_body_text_mixed_review(*, audit_csv_path: Path) -> dict[str, Any]:
    audit_rows = read_csv(audit_csv_path)
    target_rows = [row for row in audit_rows if is_medium_body_text_mixed(row)]
    ...
    return {
        ...
        "summary": {
            ...
            "target_medium_body_text_mixed_count": len(target_rows),
            "reviewed_item_count": len(items),
            "review_recommendation_counts": count_by(items, "review_recommendation"),
            "body_mixed_subcategory_counts": count_by(items, "body_mixed_subcategory"),
        ...
        "notes": [
            ...
            "Body text conflicts mixed with location/time/type conflicts require manual narrative and source review.",
        ],
    }


def is_medium_body_text_mixed(row: dict[str, str]) -> bool:
    flags = set(split_pipe(row.get("risk_flags")))
    body_flags = {"description_text_conflict", "summary_text_conflict"}
    excluded = {"same_source_multiple_native_ids", "coordinate_span_gt_5km"}
    return (
        clean_text(row.get("risk_level")) == "medium"
        and bool(flags & body_flags)
        and not flags & excluded
        and flags != body_flags
    )


def review_row(row: dict[str, str]) -> dict[str, Any]:
    flags = set(split_pipe(row.get("risk_flags")))
    has_location = "location_text_conflict" in flags
    has_classification = bool({"type_conflict", "shape_conflict"} & flags)
    has_time = "time_raw_conflict" in flags
    if has_location and has_classification:
        subcategory = "body_location_classification_time_mixed"
    elif has_location:
        subcategory = "body_location_time_mixed"
    elif has_classification:
        subcategory = "body_classification_time_mixed"
    elif has_time:
        subcategory = "body_time_mixed"
    else:
        subcategory = "body_mixed_other"
    failed_conditions = ["manual_body_mixed_review_required"]
    return {
        "review_rank": None,
        "replacement_event_id": clean_text(row.get("replacement_event_id")),
        "review_recommendation": NEEDS_DEEPER_BODY_MIXED_REVIEW,
        "body_mixed_subcategory": subcategory,
        "confidence": "low",
        ...
        "review_reason_codes": ["needs_deeper_body_mixed_review", subcategory] + failed_conditions,
...
def review_sort_key(item: dict[str, Any]) -> tuple[str, int, str]:
    return (item["body_mixed_subcategory"], -int(item["projected_event_reduction"]), item["replacement_event_id"])
...
def render_markdown(report: dict[str, Any], *, limit: int = 100) -> str:
    ...
    lines = [
        "# Medium Body-Text-Mixed Review",
        "",
        "This report is review-only. It does not accept decisions or mutate canonical outputs.",
        "",
        "## Summary",
        "",
        f"- Target items: {report['summary']['target_medium_body_text_mixed_count']}",
        f"- Recommendations: {json.dumps(report['summary']['review_recommendation_counts'], sort_keys=True)}",
        f"- Subcategories: {json.dumps(report['summary']['body_mixed_subcategory_counts'], sort_keys=True)}",
        ...
        "## Top Items",
        "",
        "| Recommendation | Subcategory | Replacement | Sources | Locations | Times | Types |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    escape_md(clean_text(item.get("body_mixed_subcategory"))),
                    f"`{escape_md(clean_text(item.get('replacement_event_id')))}`",
                    escape_md("|".join(item.get("source_file_values") or [])),
            ...
            writer.writerow(
                {
                    ...
                    "body_mixed_subcategory": item.get("body_mixed_subcategory"),
...
def main() -> int:
    args = parse_args()
    report = build_medium_body_text_mixed_review(audit_csv_path=args.audit_csv)
        ...
        json.dumps(
            {
                ...
                "target_items": report["summary"]["target_medium_body_text_mixed_count"],
                "recommendations": report["summary"]["review_recommendation_counts"],
                "subcategories": report["summary"]["body_mixed_subcategory_counts"],
                "canonical_outputs_mutated": False,
            },
            indent=2,
            ensure_ascii=False,
