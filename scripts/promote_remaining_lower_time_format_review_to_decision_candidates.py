"""Promote remaining lower-risk time-format review rows to decision candidates.

This consumes the report-only remaining lower time-format review and writes a
separate JSONL of same-event decision candidates for rows that passed strict
source review. It is intentionally not an apply step.
"""
...
DEFAULT_DECISIONS_OUTPUT = Path("data/reports/entity_resolution_remaining_lower_time_format_decision_candidates.jsonl")
DEFAULT_REPORT_OUTPUT = Path("data/reports/entity_resolution_remaining_lower_time_format_decision_candidates_report.json")

INPUT_REVIEW_POLICY = "entity_resolution_remaining_lower_time_format_source_review_only"
PROMOTION_POLICY = "entity_resolution_remaining_lower_time_format_decision_candidates_only"
DEFAULT_REVIEWER = "codex_remaining_lower_time_format_source_review_v1"
SOURCE_REVIEW_SAME_EVENT = "source_review_same_event_candidate"


def build_remaining_lower_time_format_decision_candidates(
    ...
    *,
    ...
    report = {
        ...
        "notes": [
            "These are decision candidates derived from the remaining lower time-format source-review report.",
            "They are not accepted canonical decisions and are not canonical apply.",
            "Only source-reviewed same-event candidates with no failed conditions are promoted by this lane.",
        ],
    }
...
def validate_review_report_safety(report: dict[str, Any]) -> None:
    ...
    if errors:
        raise ValueError("remaining lower time-format review report is unsafe for promotion: " + "; ".join(errors))
...
def decision_candidate_from_review_item(
    ...
    *,
    ...
    return {
        ...
        "review_type": "entity_resolution_remaining_lower_time_format_candidate",
        "decision": "same_event",
        "effect_status": "source_reviewed_candidate_not_applied",
        "decision_source": INPUT_REVIEW_POLICY,
        "promotion_policy": PROMOTION_POLICY,
        "canonical_outputs_mutated": False,
        "review_band": "remaining_lower_time_format_source_review",
        ...
        "evidence": {
            ...
            "coordinate_values": string_list(item.get("coordinate_values")),
        },
    }
...
def main() -> int:
    args = parse_args()
    decision_candidates, report = build_remaining_lower_time_format_decision_candidates(
