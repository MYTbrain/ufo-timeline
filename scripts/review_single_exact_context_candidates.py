"""Review single exact-time rows with fuzzy/context time tokens.

This consumes the source-evidence packet for
``single_exact_minute_with_context_tokens`` and produces recommendation-only
review output. It does not create decisions, preview output, accepted records,
...
DEFAULT_PACKET = Path("data/reports/entity_resolution_single_exact_context_source_evidence_packet.json")
DEFAULT_JSON_OUTPUT = Path("data/reports/entity_resolution_single_exact_context_review.json")
DEFAULT_CSV_OUTPUT = Path("data/reports/entity_resolution_single_exact_context_review.csv")
DEFAULT_MARKDOWN_OUTPUT = Path("data/reports/entity_resolution_single_exact_context_review.md")

INPUT_PACKET_POLICY = "entity_resolution_single_exact_context_source_evidence_review_only"
...
def build_single_exact_context_review(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": [
            ...
            "source_review_same_event_candidate requires one exact clock minute plus only compatible fuzzy labels, with time-only conflicts and identical source summaries.",
            "Ambiguous/unknown context tokens, non-time conflicts, incompatible fuzzy labels, or multiple summaries remain needs_more_evidence.",
        ],
    }
...
def validate_packet_safety(packet: dict[str, Any]) -> None:
    ...
    if errors:
        raise ValueError("single exact context packet is unsafe for review: " + "; ".join(errors))
...
def review_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_minutes = int_list(source.get("parsed_minutes"))
    fuzzy_labels = string_list(source.get("fuzzy_labels"))
    exact_clock_minutes = [minute for token in time_tokens if (minute := parse_exact_clock_token(token)) is not None]
    summary_texts = normalized_summary_texts(item)
    conditions = {
        "classification_is_single_exact_context": clean_text(source.get("time_pattern_classification")) == "single_exact_minute_with_context_tokens",
        "review_risk_tier_medium": clean_text(source.get("review_risk_tier")) == "medium",
        "time_only_conflict": active_conflicts == ["time"],
        "one_parsed_exact_minute": len(exact_minutes) == 1,
        "has_exact_clock_token_for_minute": len(exact_minutes) == 1 and exact_minutes[0] in exact_clock_minutes,
        "has_fuzzy_labels": bool(fuzzy_labels),
        ...
        "all_fuzzy_labels_compatible": len(exact_minutes) == 1 and all(label_is_compatible(exact_minutes[0], label) for label in fuzzy_labels),
    ...
    reason_codes = (
        [
            "source_review_exact_time_with_compatible_context",
            "time_only_conflict",
    ...
    return {
        ...
        "parsed_exact_minute": exact_minutes[0] if len(exact_minutes) == 1 else None,
        "exact_clock_minutes": sorted(set(exact_clock_minutes)),
        ...
        "notes": (
            "Exact clock time is compatible with the attached fuzzy/context labels; still requires explicit decision and apply gates."
            if same_event
...
def parse_exact_clock_token(token: str) -> int | None:
    value = clean_text(token).lower()
    if any(marker in value for marker in ("+", "?", "~")):
        return None
    compact = re.fullmatch(r"([01]\d|2[0-3])([0-5]\d)", value)
    if compact:
        return int(compact.group(1)) * 60 + int(compact.group(2))
    colon = re.fullmatch(r"([01]?\d|2[0-3]):([0-5]\d)", value)
    if colon:
        return int(colon.group(1)) * 60 + int(colon.group(2))
    am_pm = re.fullmatch(r"([1-9]|1[0-2])(?::([0-5]\d))?\s*([ap])m?", value)
    if am_pm:
...
def csv_row(item: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "parsed_exact_minute": item.get("parsed_exact_minute"),
...
def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Single Exact Context Source Review",
        "",
    ...
    for item in payload.get("items") or []:
        ...
        lines.extend(
            [
                ...
                f"- Parsed exact minute: `{item.get('parsed_exact_minute')}`",
...
def main() -> int:
    args = parse_args()
    report = build_single_exact_context_review(read_json(args.packet))
