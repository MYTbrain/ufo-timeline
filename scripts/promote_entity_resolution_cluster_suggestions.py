"""Promote ER cluster suggestions into separate AI-accepted decision records.

This does not validate decisions, plan effects, apply effects, or mutate
canonical event outputs. It only writes a separate cluster decision JSONL that
must be passed through validate_entity_resolution_cluster_decisions.py.
"""
...
DEFAULT_SUGGESTIONS = Path("data/reports/entity_resolution_cluster_review_suggestions.jsonl")
DEFAULT_SUGGESTIONS_REPORT = Path("data/reports/entity_resolution_cluster_review_suggestions_report.json")
DEFAULT_DECISIONS_OUTPUT = Path("data/canonical_full/entity_resolution_cluster_decisions_ai_accepted.jsonl")
DEFAULT_REPORT_OUTPUT = Path("data/reports/entity_resolution_cluster_suggestion_promotion_report.json")
DEFAULT_REVIEWER = "codex_ai_entity_resolution_cluster_accepted_v1"
ALLOWED_DECISIONS = {"same_event", "distinct_events", "needs_more_evidence"}


def promote_entity_resolution_cluster_suggestions(
    ...
    *,
    ...
    for index, suggestion in enumerate(suggestions, start=1):
        cluster_review_id = clean_text(suggestion.get("cluster_review_id"))
        ...
        if not cluster_review_id:
            skipped.append({"suggestion_index": index, "error": "missing_cluster_review_id"})
            continue
        ...
        decisions.append(
            {
                ...
                "notes": f"Promoted AI-assisted ER cluster suggestion ({confidence} confidence): {rationale}",
            }
        )

    report = {
        "schema_version": 1,
        "promotion_policy": "entity_resolution_cluster_suggestion_promotion_to_ai_accepted_decisions",
        ...
        "notes": [
            "Promoted cluster decisions are not validated and not applied.",
            "Run validate_entity_resolution_cluster_decisions.py before planning effects.",
            "Only a separate AI-accepted cluster decision file is written; canonical event outputs are unchanged.",
        ],
    }
...
def validate_suggestions_report(report: dict[str, Any]) -> None:
    errors: list[str] = []
    if report.get("suggestion_policy") != "entity_resolution_cluster_ai_assisted_conservative_suggestions":
        errors.append("suggestion_policy must be 'entity_resolution_cluster_ai_assisted_conservative_suggestions'")
    ...
    if errors:
        raise ValueError(f"cluster suggestions report is not safe to promote: {'; '.join(errors)}")
