"""Promote likely time-format source review rows to decision candidates.

This consumes the review-only likely time-format report and writes a separate
...
DEFAULT_REVIEW = Path("data/reports/entity_resolution_cluster_likely_time_format_review.json")
DEFAULT_DECISIONS_OUTPUT = Path("data/reports/entity_resolution_cluster_likely_time_format_decision_candidates.jsonl")
DEFAULT_REPORT_OUTPUT = Path("data/reports/entity_resolution_cluster_likely_time_format_decision_candidates_report.json")

INPUT_REVIEW_POLICY = "entity_resolution_likely_time_format_source_review_only"
PROMOTION_POLICY = "entity_resolution_likely_time_format_decision_candidates_only"
DEFAULT_REVIEWER = "codex_likely_time_format_source_review_v1"
SOURCE_REVIEW_SAME_EVENT = "source_review_same_event_candidate"


def build_likely_time_format_decision_candidates(
    ...
    *,
    ...
    report = {
        ...
        "notes": [
            "These are decision candidates derived from the likely time-format source-review report.",
            "They are not canonical apply; use effects-plan and dry-run gates before any candidate corpus rewrite.",
            "Only bare-hour versus exact-clock same-minute source rows are promoted by this lane.",
        ],
    }
...
def validate_review_report_safety(report: dict[str, Any]) -> None:
    ...
    if errors:
        raise ValueError("likely time-format review report is unsafe for promotion: " + "; ".join(errors))
    ...
    *,
    ...
    return {
        ...
        "evidence": {
            ...
            "token_kinds": item.get("token_kinds") if isinstance(item.get("token_kinds"), dict) else {},
...
def main() -> int:
    args = parse_args()
    decision_candidates, report = build_likely_time_format_decision_candidates(
