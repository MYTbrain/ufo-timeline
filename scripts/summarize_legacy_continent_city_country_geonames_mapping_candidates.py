"""Find GeoNames matches for legacy city/admin/country/continent rows.

Some legacy residuals are shaped like ``Benevides, Para, BRA, SA`` where the
final token is a continent marker and the penultimate token is the real country
code. This report-only lane ignores the trailing continent marker and accepts
only unique or strongly dominant populated-place matches within the country.
"""
...
DEFAULT_MAPPING_CSV = Path("data/reports/mapping_coverage_opportunities_after_washington_dc_city_state_v15.csv")
...
DEFAULT_OUTPUT_JSON = Path("data/reports/legacy_continent_city_country_geonames_mapping_candidates_after_washington_dc_v16.json")
DEFAULT_OUTPUT_CSV = Path("data/reports/legacy_continent_city_country_geonames_mapping_candidates_after_washington_dc_v16.csv")

CONTINENT_CODES = {"aa", "af", "an", "as", "au", "eu", "na", "oc", "p", "sa"}
BROAD_PLACE_TOKENS = {
    "antarctica",
    "atlantic ocean",
    "australia",
...
def summarize_legacy_continent_city_country_geonames_mapping_candidates(
    *,
    mapping_csv: Path,
    geonames_zip: Path,
    country_info: Path,
    ...
    return {
        "schema_version": 1,
        "report_policy": "legacy_continent_city_country_geonames_mapping_candidates_report_only",
        ...
        "parseable_legacy_continent_city_country_query_count": len(parsed_queries),
        ...
        "notes": [
            "Only city/admin/country/continent residual rows are considered.",
            "The trailing continent marker is ignored; the penultimate token must resolve as a country.",
            "Broad country, state, ocean, and region tokens are ignored.",
            "Matching uses GeoNames primary/ascii populated-place names only.",
...
def parse_query(row: dict[str, str], country_aliases: dict[str, str]) -> dict[str, Any] | None:
    ...
    if len(parts) < 4 or parts[-1] not in CONTINENT_CODES:
        return None
    country_code = country_aliases.get(parts[-2])
    if not country_code:
        return None
    city = clean_city(parts[0])
    if not city or "/" in city or city in BROAD_PLACE_TOKENS:
        return None
    ...
    if city_key(city) in country_alias_keys:
        return None
    return {
        "query": query,
