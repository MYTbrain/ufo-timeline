"""Promote single exact/context source review rows to decision candidates.

This consumes the review-only single exact/context report and writes a separate
JSONL of same-event decision candidates for rows that passed strict source
...
from scripts.review_single_exact_context_candidates import parse_exact_clock_token


DEFAULT_REVIEW = Path("data/reports/entity_resolution_single_exact_context_review.json")
DEFAULT_DECISIONS_OUTPUT = Path("data/reports/entity_resolution_single_exact_context_decision_candidates.jsonl")
DEFAULT_REPORT_OUTPUT = Path("data/reports/entity_resolution_single_exact_context_decision_candidates_report.json")

INPUT_REVIEW_POLICY = "entity_resolution_single_exact_context_source_review_only"
PROMOTION_POLICY = "entity_resolution_single_exact_context_decision_candidates_only"
DEFAULT_REVIEWER = "codex_single_exact_context_source_review_v1"
SOURCE_REVIEW_SAME_EVENT = "source_review_same_event_candidate"


def build_single_exact_context_decision_candidates(
    ...
    *,
    ...
    for index, item in enumerate(items, start=1):
        ...
        if len(merge_event_ids) < 2:
            ...
            continue
        parsed_minute = item.get("parsed_exact_minute")
        clean_exact_tokens = clean_exact_clock_tokens_for_minute(item, parsed_minute)
        if not clean_exact_tokens:
            skipped.append(skip_record(index, item, "missing_clean_exact_clock_token"))
            continue
        decision_candidates.append(
            decision_candidate_from_review_item(
                ...
                clean_exact_clock_tokens=clean_exact_tokens,
            )
        )
    report = {
        "schema_version": 1,
        ...
        "notes": [
            "These are decision candidates derived from the single exact/context source-review report.",
            "They are not canonical apply; use effects-plan and dry-run gates before any candidate corpus rewrite.",
            "Modifier-bearing tokens are recorded but not counted as clean exact-clock evidence.",
        ],
    }
...
def validate_review_report_safety(report: dict[str, Any]) -> None:
    ...
    if errors:
        raise ValueError("single exact/context review report is unsafe for promotion: " + "; ".join(errors))
...
def decision_candidate_from_review_item(
    ...
    *,
    ...
    reviewer: str,
    reviewed_at: str,
    clean_exact_clock_tokens: list[str],
) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence": {
            ...
            "modifier_tokens": modifier_tokens(item),
            "clean_exact_clock_tokens": clean_exact_clock_tokens,
            "parsed_exact_minute": item.get("parsed_exact_minute"),
            "exact_clock_minutes": int_list(item.get("exact_clock_minutes")),
...
def clean_exact_clock_tokens_for_minute(item: dict[str, Any], parsed_minute: Any) -> list[str]:
    try:
        minute = int(parsed_minute)
    except (TypeError, ValueError):
        return []
    tokens: list[str] = []
    for token in string_list(item.get("time_tokens")):
        if has_modifier(token):
            continue
        if parse_exact_clock_token(token) == minute:
            tokens.append(token)
    return tokens


def modifier_tokens(item: dict[str, Any]) -> list[str]:
    return [token for token in string_list(item.get("time_tokens")) if has_modifier(token)]


def has_modifier(token: str) -> bool:
    return any(marker in clean_text(token) for marker in ("+", "?", "~"))
...
def main() -> int:
    args = parse_args()
    decision_candidates, report = build_single_exact_context_decision_candidates(
