"""Accept source-reviewed single exact/context candidates.

This creates accepted decision records for the single exact/context lane only
after preview output and canonical-body dry-run checks are valid. It does not
apply merges or rewrite canonical event outputs.
"""
...
DEFAULT_DECISION_CANDIDATES = Path("data/reports/entity_resolution_single_exact_context_decision_candidates.jsonl")
DEFAULT_DECISION_CANDIDATE_REPORT = Path("data/reports/entity_resolution_single_exact_context_decision_candidates_report.json")
DEFAULT_PREVIEW_REPORT = Path("data/reports/entity_resolution_single_exact_context_preview_apply_report.json")
DEFAULT_PREVIEW_OUTPUT_CHECK = Path("data/reports/entity_resolution_single_exact_context_preview_output_check.json")
DEFAULT_CANONICAL_BODY_DRY_RUN_CHECK = Path("data/reports/entity_resolution_single_exact_context_canonical_body_dry_run_check.json")
DEFAULT_ACCEPTED_DECISIONS_OUTPUT = Path("data/canonical_full/entity_resolution_single_exact_context_accepted_decisions.jsonl")
DEFAULT_REPORT_OUTPUT = Path("data/reports/entity_resolution_single_exact_context_acceptance_report.json")

INPUT_PROMOTION_POLICY = "entity_resolution_single_exact_context_decision_candidates_only"
...
DEFAULT_REVIEWER = "codex_single_exact_context_policy_acceptance_v1"


def accept_single_exact_context_decisions(
    *,
    ...
    if invalid:
        raise ValueError("single exact/context decision candidates are unsafe for acceptance: " + json.dumps(invalid[:5], ensure_ascii=False))
    ...
    report = {
        ...
        "notes": [
            "Accepted decisions are policy-gated from the source-reviewed single exact/context lane only.",
...
def accepted_decision_from_candidate(candidate: dict[str, Any], *, reviewer: str, accepted_at: str) -> dict[str, Any]:
    ...
    accepted["notes"] = append_note(
        accepted.get("notes"),
        "Policy-accepted after valid preview output and valid canonical-body dry-run checks for the source-reviewed single exact/context lane.",
    )
    return accepted
...
def validate_candidate_report(report: dict[str, Any], *, expected_count: int) -> None:
    ...
    if errors:
        raise ValueError("decision candidate report is unsafe for single exact/context acceptance: " + "; ".join(errors))
...
def validate_preview_report(payload: dict[str, Any], *, expected_count: int) -> None:
    ...
    if errors:
        raise ValueError("preview report is unsafe for single exact/context acceptance: " + "; ".join(errors))
...
def validate_preview_output_check(payload: dict[str, Any], *, expected_count: int) -> None:
    ...
    for flag in ("canonical_outputs_mutated", "preview_outputs_written", "auto_merge_performed"):
        if payload.get(flag) is not False:
            errors.append(f"{flag} must be false")
    if errors:
        raise ValueError("preview output check is unsafe for single exact/context acceptance: " + "; ".join(errors))
...
def validate_canonical_body_dry_run_check(payload: dict[str, Any], *, expected_count: int) -> None:
    ...
    for flag in ("canonical_outputs_mutated", "preview_outputs_written", "decisions_created", "auto_merge_performed", "ready_for_canonical_apply"):
        if payload.get(flag) is not False:
            errors.append(f"{flag} must be false")
    if errors:
        raise ValueError("canonical body dry-run check is unsafe for single exact/context acceptance: " + "; ".join(errors))
...
def validate_decision_candidate(candidate: dict[str, Any]) -> str | None:
    ...
    if clean_text(candidate.get("review_band")) != "strict_single_exact_context_source_review":
    ...
    if not string_list(evidence.get("clean_exact_clock_tokens")):
        return "clean_exact_clock_tokens_required"
    if any(token in string_list(evidence.get("clean_exact_clock_tokens")) for token in string_list(evidence.get("modifier_tokens"))):
        return "modifier_tokens_must_not_be_clean_exact_clock_tokens"
    if not int_list(evidence.get("exact_clock_minutes")):
        return "exact_clock_minutes_required"
    try:
        parsed_exact_minute = int(evidence.get("parsed_exact_minute"))
    except (TypeError, ValueError):
        return "parsed_exact_minute_required"
    if parsed_exact_minute not in int_list(evidence.get("exact_clock_minutes")):
        return "parsed_exact_minute_must_match_exact_clock_minutes"
    return None
