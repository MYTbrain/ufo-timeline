"""Accept source-reviewed likely time-format candidates.

This creates accepted decision records for the likely time-format lane only
...
DEFAULT_DECISION_CANDIDATES = Path("data/reports/entity_resolution_cluster_likely_time_format_decision_candidates.jsonl")
DEFAULT_DECISION_CANDIDATE_REPORT = Path(
    "data/reports/entity_resolution_cluster_likely_time_format_decision_candidates_report.json"
)
DEFAULT_PREVIEW_REPORT = Path("data/reports/entity_resolution_cluster_likely_time_format_preview_apply_report.json")
DEFAULT_PREVIEW_OUTPUT_CHECK = Path("data/reports/entity_resolution_cluster_likely_time_format_preview_output_check.json")
DEFAULT_CANONICAL_BODY_DRY_RUN_CHECK = Path(
    "data/reports/entity_resolution_cluster_likely_time_format_canonical_body_dry_run_check.json"
)
...
DEFAULT_REPORT_OUTPUT = Path("data/reports/entity_resolution_cluster_likely_time_format_acceptance_report.json")

INPUT_PROMOTION_POLICY = "entity_resolution_likely_time_format_decision_candidates_only"
...
DEFAULT_REVIEWER = "codex_likely_time_format_policy_acceptance_v1"


def accept_likely_time_format_decisions(
    *,
    ...
    if invalid:
        raise ValueError("likely time-format decision candidates are unsafe for acceptance: " + json.dumps(invalid[:5], ensure_ascii=False))
    ...
    report = {
        ...
        "notes": [
            "Accepted decisions are policy-gated from the source-reviewed likely time-format lane only.",
...
def accepted_decision_from_candidate(candidate: dict[str, Any], *, reviewer: str, accepted_at: str) -> dict[str, Any]:
    ...
    accepted["notes"] = append_note(
        accepted.get("notes"),
        "Policy-accepted after valid preview output and valid canonical-body dry-run checks for the source-reviewed likely time-format lane.",
    )
    return accepted
...
def validate_candidate_report(report: dict[str, Any], *, expected_count: int) -> None:
    ...
    if int(report.get("skipped_review_item_count") or 0) != 0:
        errors.append("skipped_review_item_count must be 0 for the likely time-format source-review cases")
    ...
    if errors:
        raise ValueError("decision candidate report is unsafe for likely time-format acceptance: " + "; ".join(errors))
...
def validate_preview_report(payload: dict[str, Any], *, expected_count: int) -> None:
    ...
    if errors:
        raise ValueError("preview report is unsafe for likely time-format acceptance: " + "; ".join(errors))
...
def validate_preview_output_check(payload: dict[str, Any], *, expected_count: int) -> None:
    ...
    for flag in ("canonical_outputs_mutated", "preview_outputs_written", "auto_merge_performed"):
        if payload.get(flag) is not False:
            errors.append(f"{flag} must be false")
    if errors:
        raise ValueError("preview output check is unsafe for likely time-format acceptance: " + "; ".join(errors))
...
def validate_canonical_body_dry_run_check(payload: dict[str, Any], *, expected_count: int) -> None:
    ...
    for flag in ("canonical_outputs_mutated", "preview_outputs_written", "decisions_created", "auto_merge_performed", "ready_for_canonical_apply"):
        if payload.get(flag) is not False:
            errors.append(f"{flag} must be false")
    if errors:
        raise ValueError("canonical body dry-run check is unsafe for likely time-format acceptance: " + "; ".join(errors))
...
def validate_decision_candidate(candidate: dict[str, Any]) -> str | None:
    ...
    if clean_text(candidate.get("review_band")) != "strict_likely_time_format_source_review":
