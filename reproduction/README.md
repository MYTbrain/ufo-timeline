...
3. the immutable, versioned R2 objects recorded with exact byte counts and
   SHA-256 hashes in `release.json`.
...
R2 for application data; production R2 URLs in `data/app_config.json` are
localized to the verified downloaded files.
...
- Downloads use TLS verification and the operating-system trust store.
